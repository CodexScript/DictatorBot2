...
async function createNewQuiz(
    ...
    useThread: boolean,
    visible: boolean,
): Promise<BasedometerInstance | null> {
    if (
    ...
    .addSubcommand((subcommand) =>
        subcommand
            ...
            .addNumberOption((option) =>
                option.setName('rating').setDescription('Your rating for these images').setRequired(true),
            ),
    );
    ...
    if (interaction.options.getSubcommand() === 'start') {
        ...
        if (existingInstance !== undefined) {
            ...
            if (interaction.channel instanceof TextChannel) {
                instanceLocation = `#${interaction.channel.name} in ${
                    interaction.guild?.name ? interaction.guild.name : 'a group DM'
                }`;
        ...
        if (instance === undefined) {
            await interaction.reply({
                content: 'You must have an active test in order to use that command.',
                ephemeral: true,
            });
            return;
        }
        ...
        await interaction.reply({
            content: `You rated ${entry.name}: ***${rating}***\nThe CCP-approved rating for ${entry.name}: *${entry.baseRating}*\nDifference: *${diff}*`,
            ephemeral: !instance.visible,
        });
