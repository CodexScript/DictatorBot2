...
export const data = new SlashCommandBuilder().setName('ussifier').setDescription('Ussifies.');
...
export async function execute(interaction: CommandInteraction): Promise<void> {
    ...
    if (!alphaWorld) {
        await interaction.reply({
            content:
                "Fetching Alpha World server from cache failed. I'm probably not in the server. If you are 100% positive that I am, try sending a message in the general channel, then try again.",
            ephemeral: true,
        });
        return;
    }
    ...
    for (const memberArr of members) {
        ...
        if (
            ...
            (member.nickname === null || member.nickname.endsWith('ussy') === false)
        ) {
