...
export const data = new SlashCommandBuilder()
  ...
  .addIntegerOption((option) => option.setName('free_slots')
    .setDescription('The number of free slots to find. Default: 2')
    .setRequired(false))
  .addIntegerOption((option) => option.setName('min_players')
    .setDescription('The minimum number of players in the server. Default: 15')
    .setRequired(false))
  .addIntegerOption((option) => option.setName('distance')
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  for (const server of servers.data) {
    if ((server.a2s.MaxPlayers - server.a2s.Players) >= freeSlots && (minPlayers !== 0 && server.a2s.Players >= minPlayers) && (distance !== 0 && coordsDistance(40.65965, -73.54340, server.latitude, server.longitude) <= distance)) {
      await interaction.followUp({ content: `Connect using the following URL: steam://connect/${server.host}:${server.port}` });
      return;
    }
  }
}
