...
export async function setSocialCredit(client: BetterSqlite3.Database, userID: string, socialCredit: number) {
    // await client.set(`social_credit:${userID}`, socialCredit);
    return;
}

export async function getSocialCredit(client: BetterSqlite3.Database, userID: string) {
...
export async function addSocialCredit(client: BetterSqlite3.Database, userID: string, socialCredit: number) {
...
export async function createUserBanner(client: BetterSqlite3.Database, member: GuildMember): Promise<Buffer> {
    ...
    const username = `${member.nickname ? member.nickname : member.user.username} Citizen no. ${
